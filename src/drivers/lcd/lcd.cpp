...
size_t lcd_write(const char *buf, size_t size)
{
    ...
    while (lines > 0) {
        if (lines == 1) {
            ...
            // lcd.print((buf + offset), to_write);
            lcd.print(str.substring(offset, (offset + to_write) - 1));
            to_write -= to_write;
        } else {
            lcd.setCursor(0, cur_line);
            // lcd.write((buf + offset), WIDTH);
            lcd.print(str.substring(offset, ((cur_line + 1) * WIDTH) - 1));
            to_write -= WIDTH;
            offset += WIDTH;
            cur_line += 1;
        }
